 ...
 * MODIFICADO: Sidebar visible en móviles (eliminado div contenedor .sidebar-column)
 * MODIFICADO: Agregado tema claro/oscuro (como en audit.php) incluyendo avisos/alertas
 */
...
// ============================================
// FUNCIONES PARA OBTENER MEMBRESÍAS POR ESTADO
// ============================================
...
function getMembershipsByStatus($pdo, $status, $limit, $offset, $search) {
    ...
    $query = "SELECT * FROM memberships $whereClause ORDER BY sort_order ASC, price ASC LIMIT $limit OFFSET $offset";
    $stmt = $pdo->prepare($query);
...
function getTotalMembershipsByStatus($pdo, $status, $search) {
    ...
    $query = "SELECT COUNT(*) as total FROM memberships $whereClause";
    $stmt = $pdo->prepare($query);
    foreach ($params as $key => $value) {
        $stmt->bindValue($key, $value);
    }
...
// Obtener estadísticas para los tabs
...
// Obtener membresías según el tab activo
...
$totalPages = ceil($total / $current_limit);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .btn-icon.status:hover { background: #ffc107; color: #333; }
        .btn-icon.featured:hover { background: #f9ca24; color: #333; }
        ...
        /* ============================================
           ESTILOS PARA ALERTAS/AVISOS CON TEMA OSCURO
           ============================================ */
        .alert {
            margin: 15px 20px 0;
            padding: 10px 15px;
        ...
        /* ============================================
           RESPONSIVE - SIDEBAR VISIBLE EN MÓVILES
           ============================================ */
...
<div class="admin-container">
    <!-- SIDEBAR - INCLUIDO DIRECTAMENTE SIN DIV CONTENEDOR (sidebar visible en móviles) -->
        ...
        <div class="section-container">
            ...
            <div class="membership-tabs">
                <button class="tab-btn <?php echo $tab_actual == 'activas' ? 'active' : ''; ?>" data-tab="activas">
                    ...
                    <span class="tab-badge"><?php echo $stats_activas; ?></span>
                </button>
                <button class="tab-btn <?php echo $tab_actual == 'inactivas' ? 'active' : ''; ?>" data-tab="inactivas">
                    <i class="fas fa-ban"></i> Inactivas
                    <span class="tab-badge"><?php echo $stats_inactivas; ?></span>
                </button>
            </div>
            ...
            <div class="filter-bar">
                <form method="GET" class="filter-form" id="filterForm">
                    ...
                    <a href="?tab=<?php echo $tab_actual; ?>&page=1" class="btn btn-secondary">Limpiar</a>
                </form>
            </div>
                        ...
                        <?php if (count($memberships) > 0): ?>
                            <?php foreach ($memberships as $membership): ?>
                                <tr id="fila_<?php echo $membership['id']; ?>">
                                    ...
                                    <td>
                                        <strong><?php echo htmlspecialchars(ucfirst($membership['name'])); ?></strong>
                                        <?php if (!empty($membership['description'])): ?>
                                            <br><small class="text-muted"><?php echo htmlspecialchars(substr($membership['description'], 0, 40)) . (strlen($membership['description']) > 40 ? '...' : ''); ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td class="price-cell">
                                    ...
                                    <td>
                                        <?php if ($membership['max_publications'] >= 999999): ?>
                                            <span class="badge badge-info">Ilimitadas</span>
                                        <?php else: ?>
                                            <?php echo number_format($membership['max_publications']); ?>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($membership['has_invoicing']): ?>
                                            <span class="badge badge-info" title="Facturación DIAN"><i class="fas fa-file-invoice"></i></span>
                                        <?php endif; ?>
                                        <?php if ($membership['has_statistics']): ?>
                                            <span class="badge badge-info" title="Estadísticas"><i class="fas fa-chart-line"></i></span>
                                        <?php endif; ?>
                                        <?php if ($membership['has_support_priority']): ?>
                                            <span class="badge badge-info" title="Soporte prioritario"><i class="fas fa-headset"></i></span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-center"><?php echo $membership['sort_order']; ?></td>
                                    <td>
                                        <?php if ($membership['is_featured'] == 1): ?>
                                            <span class="badge badge-featured"><i class="fas fa-star"></i> Destacada</span>
                                        <?php else: ?>
                                            <span class="text-muted">-</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if ($membership['active'] == 1): ?>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button class="btn-icon edit" onclick="editMembership(<?php echo $membership['id']; ?>)" title="Editar">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn-icon featured" onclick="toggleFeatured(<?php echo $membership['id']; ?>, <?php echo $membership['is_featured']; ?>)" title="<?php echo $membership['is_featured'] == 1 ? 'Quitar destacada' : 'Destacar'; ?>">
                                                <i class="fas fa-star"></i>
                                            </button>
                                            <button class="btn-icon status" onclick="toggleStatus(<?php echo $membership['id']; ?>, <?php echo $membership['active']; ?>)" title="<?php echo $membership['active'] == 1 ? 'Desactivar' : 'Activar'; ?>">
                                                <i class="fas fa-<?php echo $membership['active'] == 1 ? 'ban' : 'check-circle'; ?>"></i>
                                            </button>
                                            <button class="btn-icon delete" onclick="deleteMembership(<?php echo $membership['id']; ?>)" title="Eliminar">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="10" class="empty-state">
                                    ...
                                    <p>No hay membresías <?php echo $tab_actual == 'activas' ? 'activas' : 'inactivas'; ?>.</p>
                                    <?php if ($tab_actual == 'inactivas'): ?>
                                        <small>Las membresías que desactives aparecerán aquí.</small>
                                    <?php else: ?>
                                        <small>Haz clic en "Nueva Membresía" para crear una.</small>
                                    <?php endif; ?>
                                 </div>
                                </table>
                        <?php endif; ?>
            ...
            <div class="pagination-container">
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        $total_pages_int = (int)$totalPages;
                        $prev_page = $current_page_int - 1;
                        $next_page = $current_page_int + 1;
                        ?>
                        <li class="page-item <?php echo $current_page_int <= 1 ? 'disabled' : ''; ?>">
                            <a class="page-link" href="#" onclick="changePage(<?php echo $prev_page; ?>); return false;">Anterior</a>
                        </li>
                        <?php 
                        ...
                        if ($end_page < $total_pages_int) {
                            if ($end_page < $total_pages_int - 1) echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
                            echo '<li class="page-item"><a class="page-link" href="#" onclick="changePage(' . $total_pages_int . '); return false;">' . $total_pages_int . '</a></li>';
                        }
                        ?>
                        <li class="page-item <?php echo $current_page_int >= $total_pages_int ? 'disabled' : ''; ?>">
                            <a class="page-link" href="#" onclick="changePage(<?php echo $next_page; ?>); return false;">Siguiente</a>
                        </li>
                    </ul>
                </nav>
            </div>
...
<!-- SweetAlert2 - CDN (única librería que necesita este archivo, jQuery y Bootstrap ya están en sidebar) -->
...
<script>
    ...
    function createModal() {
        ...
        document.getElementById('membershipForm').addEventListener('submit', function(e) {
            ...
            if (!name) {
                showFormError('El nombre de la membresía es obligatorio.');
                return;
            }
            ...
            fetch(window.location.href, {
                ...
                showFormError('Error al guardar la membresía.');
            });
        });
    }
    ...
    function editMembership(id) {
        ...
        Swal.fire({
            title: 'Cargando...',
            text: 'Obteniendo datos de la membresía',
        ...
        fetch('memberships.php?action=get&id=' + id)
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    Swal.fire({
                        title: 'Error',
                        text: data.message || 'Error al cargar los datos de la membresía.',
                        icon: 'error',
    ...
    function toggleStatus(id, currentStatus) {
        ...
        Swal.fire({
            title: `${action === 'activar' ? 'Activar' : 'Desactivar'} membresía`,
            text: `¿Estás seguro de ${action} esta membresía? Pasará a estar ${newStatusText}.`,
            icon: 'question',
    ...
    function toggleFeatured(id, currentFeatured) {
        ...
        Swal.fire({
            title: `${currentFeatured == 1 ? 'Quitar destacada' : 'Destacar'} membresía`,
            text: `¿Estás seguro de ${action} esta membresía?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#c8a86b',
            cancelButtonColor: '#6c757d',
    ...
    function deleteMembership(id) {
        Swal.fire({
            ...
            text: '¿Estás seguro de eliminar esta membresía? Si tiene usuarios asociados no podrá eliminarla.',
            icon: 'warning',
            showCancelButton: true,
