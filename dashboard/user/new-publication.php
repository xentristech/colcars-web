<?php
/**
 * Colcars - Nueva Publicación
 * Ruta: /dashboard/user/new-publication.php
 * MODIFICADO: Bloquea la creación de publicaciones si la cuenta está desactivada.
...
// ============================================
// CORRECCIÓN: Usar BASE_PATH ya que está definida en config.php
...
if (!canUserPost($user_id)) {
    header('Location: my-publications.php?error=limit');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (strlen($titulo) < 10) $errors[] = 'El título debe tener al menos 10 caracteres';
        if (empty($categoria_id)) $errors[] = 'Selecciona una categoría';
        if (empty($descripcion)) $errors[] = 'La descripción es requerida';
        if (strlen($descripcion) < 50) $errors[] = 'La descripción debe tener al menos 50 caracteres';
        if ($precio <= 0) $errors[] = 'El precio debe ser mayor a 0';
        if ($year_fabricacion < 1900 || $year_fabricacion > date('Y') + 1) $errors[] = 'Año de fabricación inválido';
        ...
        if (empty($errors)) {
            try {
                ...
                // Insertar publicación con todos los campos
                $publicacion_id = $db->insert('publicaciones', [
                    'usuario_id' => $user_id,
                ...
                if ($publicacion_id) {
                    // ============================================
                    // CORRECCIÓN: Procesar imágenes con ruta ABSOLUTA usando BASE_PATH
                    // ============================================
                    if (!empty($_FILES['imagenes']['name'][0])) {
                        ...
                        if (!is_dir($absolute_user_folder)) {
                            if (!mkdir($absolute_user_folder, 0777, true)) {
                                error_log("ERROR: No se pudo crear la carpeta: " . $absolute_user_folder);
                            ...
                            if ($_FILES['imagenes']['error'][$key] === UPLOAD_ERR_OK) {
                                ...
                                if (move_uploaded_file($tmp_name, $destination)) {
                                    ...
                                    $db->insert('imagenes_publicaciones', [
                                        ...
                                        'image_path' => $url,
                                        'is_primary' => $es_principal ? 1 : 0,
                                        'sort_order' => $key
                                    ]);
                                    $es_principal = false;
                                } else {
                                    error_log("ERROR: No se pudo mover el archivo a: " . $destination);
                                }
                            }
                        }
                    }
                    ...
                    if (!empty($_FILES['documentos']['name'][0])) {
                        ...
                        if (!is_dir($absolute_docs_folder)) {
                            if (!mkdir($absolute_docs_folder, 0777, true)) {
                                error_log("ERROR: No se pudo crear la carpeta de documentos: " . $absolute_docs_folder);
                        ...
                        foreach ($_FILES['documentos']['tmp_name'] as $key => $tmp_name) {
                            if ($_FILES['documentos']['error'][$key] === UPLOAD_ERR_OK) {
                                $tipo = $_POST['documento_tipo'][$key] ?? 'otros';
                                $extension = strtolower(pathinfo($_FILES['documentos']['name'][$key], PATHINFO_EXTENSION));
                                ...
                                if (move_uploaded_file($tmp_name, $destination)) {
                                    ...
                                    error_log("ERROR: No se pudo mover el documento a: " . $destination);
                                }
                            }
                        }
                    }
                    ...
                    $db->commit();
                    logAudit($user_id, 'CREATE', 'publicaciones', $publicacion_id, null, $_POST);
                    
                    $success = 'Publicación creada exitosamente';
                    header("refresh:2;url=my-publications.php");
                } else {
                    $db->rollback();
                    $errors[] = 'Error al crear la publicación';
                }
            } catch (Exception $e) {
                $db->rollback();
...
<head>
    ...
    <title>Nueva Publicación - Colcars</title>
    ...
    <style>
        ...
        * {
        ...
        /* DOCUMENT ITEM EN MODO OSCURO */
        [data-theme="dark"] .document-item {
            background: rgba(102, 126, 234, 0.15) !important;
        }

        /* IMAGE PREVIEW EN MODO OSCURO */
...
<body>
    ...
    <div class="offcanvas offcanvas-start mobile-offcanvas" tabindex="-1" id="mobileOffcanvas">
        ...
        <div class="offcanvas-body">
            ...
            <nav class="nav flex-column">
                ...
                <a class="nav-link active" href="new-publication.php">
    ...
    <div class="dashboard-wrapper">
        ...
        <div class="content-column">
            <div class="main-content">
                ...
                <p class="text-muted">Completa todos los campos para publicar tu vehículo o parte</p>
                ...
                <form method="POST" action="" enctype="multipart/form-data" id="publicationForm">
                    ...
                    <div class="form-card">
                        ...
                        <div class="row">
                            <div class="col-md-8 mb-3">
                                ...
                                <input type="text" class="form-control" name="titulo" required 
                                       placeholder="Ej: BMW Serie 3 2024, Yamaha MT-09, etc.">
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="descripcion" rows="5" required 
                                      placeholder="Describe las características, condiciones, accesorios, etc. (mínimo 50 caracteres)"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select" name="negociable">
                                    <option value="1">Sí, negociable</option>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control" name="kilometraje" placeholder="Ej: 15000">
                            </div>
                            <div class="col-md-4 mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="ubicacion" placeholder="Ciudad, Departamento">
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        ...
                        <p class="text-muted small">Estos datos son registrados en el sistema RUNT y deben coincidir exactamente con la factura de compra, el manifiesto de aduana y las improntas.</p>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Marca</label>
                                <input type="text" class="form-control" name="brand" placeholder="Ej: Chevrolet, Toyota, BMW">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Línea o modelo comercial</label>
                                <input type="text" class="form-control" name="linea_modelo_comercial" placeholder="Ej: Corolla, Spark, X5">
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control" name="cilindrada" placeholder="Ej: 1600, 2000">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Potencia (HP)</label>
                                <input type="number" class="form-control" name="potencia_hp" placeholder="Caballos de fuerza">
                            </div>
                            <div class="col-md-4 mb-3">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" name="capacidad" placeholder="Ej: 5 pasajeros, 1200 kg">
                            </div>
                            <div class="col-md-6 mb-3">
                    ...
                    <div class="form-card">
                        ...
                        <p class="text-muted small">Son los números de identificación permanente del vehículo, los cuales son validados por el perito de tránsito.</p>
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Número de Motor</label>
                                <input type="text" class="form-control" name="numero_motor" placeholder="Número del motor">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Número de Chasis</label>
                                <input type="text" class="form-control" name="numero_chasis" placeholder="Número de chasis">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label class="form-label">Número de Serie o VIN</label>
                                <input type="text" class="form-control" name="numero_vin" placeholder="VIN (17 caracteres)">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        ...
                        <p class="text-muted small">Se asocian al vehículo al momento de su registro en la sede de tránsito.</p>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control" name="propietario_nombre" placeholder="Nombres y apellidos">
                            </div>
                            <div class="col-md-4 mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control" name="propietario_numero_documento" placeholder="Número de identificación">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Empresa Vinculadora (Nombre)</label>
                                <input type="text" class="form-control" name="empresa_vinculadora_nombre" placeholder="Solo para servicio público">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Empresa Vinculadora (NIT)</label>
                                <input type="text" class="form-control" name="empresa_vinculadora_nit" placeholder="NIT de la empresa">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-check">
                            ...
                            <small class="d-block text-muted">Los documentos legales solo serán visibles para usuarios con membresía Premium o Elite</small>
                        </div>
                    </div>

                    <!-- Imágenes -->
                    <div class="form-card">
                        <h5><i class="fas fa-images"></i> Imágenes del Vehículo</h5>
                        <p class="text-muted">Puedes subir hasta 10 imágenes. La primera será la principal.</p>
                    ...
                    <!-- Documentación legal -->
                    <div class="form-card">
                        <h5><i class="fas fa-file-alt"></i> Documentación Legal</h5>
                        <p class="text-muted">Sube los documentos del vehículo (SOAT, Tarjeta de Propiedad, Técnico-mecánica, etc.)</p>
                        ...
                        <div id="documentosContainer">
                            <div class="document-item">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <input type="file" class="form-control" name="documentos[]" accept=".pdf,.jpg,.png">
                                    </div>
                                    <div class="col-md-2">
                                        <button type="button" class="btn btn-danger btn-sm" onclick="removeDocument(this)">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary mt-2" onclick="addDocument()">
    ...
    <script>
            ...
            for (let i = 0; i < this.files.length; i++) {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = function(e) {
                        ...
                        div.innerHTML = `<img src="${e.target.result}" alt="Preview"><button type="button" class="remove-btn" onclick="removeImage(this, ${i})">×</button>`;
        ...
        function addDocument() {
            ...
            newDoc.innerHTML = `<div class="row mt-2"><div class="col-md-4"><select class="form-select" name="documento_tipo[]"><option value="soat">SOAT</option><option value="tecnicomecanica">Técnico-mecánica</option><option value="tarjeta_propiedad">Tarjeta de Propiedad</option><option value="factura_compra">Factura de Compra</option><option value="otros">Otros</option></select></div><div class="col-md-6"><input type="file" class="form-control" name="documentos[]" accept=".pdf,.jpg,.png"></div><div class="col-md-2"><button type="button" class="btn btn-danger btn-sm" onclick="removeDocument(this)"><i class="fas fa-trash"></i></button></div></div>`;
            container.appendChild(newDoc);
        }
        
        function removeDocument(btn) {
            const container = document.getElementById('documentosContainer');
            if (container.children.length > 1) btn.closest('.document-item').remove();
        }
    </script>
</body>
</html>